...
/** Pipeline ra quyết định dùng chung cho Dashboard và dữ liệu xuất Excel. */
export const DECISION_STATES = Object.freeze([
  { value: 'NEW', label: 'Mới phát hiện' },
  { value: 'REVIEW', label: 'Đang sàng lọc' },
  { value: 'GO', label: 'Quyết định dự thầu' },
  { value: 'BID', label: 'Đang lập HSDT' },
  { value: 'SUBMITTED', label: 'Đã nộp' },
  { value: 'NO_GO', label: 'Không tham gia' }
]);

export const DECISION_STATE_LABEL = Object.freeze(
  Object.fromEntries(DECISION_STATES.map((x) => [x.value, x.label]))
);

export function normalizeDecisionState(value) {
  const v = String(value || '').toUpperCase();
  return DECISION_STATE_LABEL[v] ? v : 'NEW';
}
...
/**
 * Độ đầy đủ của dữ liệu dùng để RA QUYẾT ĐỊNH, không phải xác suất đúng.
 * Điểm này cố ý không "đoán" từ nội dung; trường nào có nguồn thì cộng trường đó.
 */
export function dataConfidence(t) {
  const st = statusOf(t);
  let score = 0;
  if (t && (t.notifyNo || t.bidNo)) score += 18;
  if (t && t.bidName) score += 12;
  if (t && t.price !== null && t.price !== undefined && t.price !== '') score += 16;
  if (st === 'PLAN' || (t && t.closeDate)) score += 16;
  if (t && t.location) score += 12;
  if (t && (t.investorName || t.procuringEntityName)) score += 12;
  if (t && /^https:\/\/muasamcong\.mpi\.gov\.vn\//i.test(String(t.detailUrl || ''))) score += 14;
  const value = Math.min(100, score);
  const level = value >= 85 ? 'GOOD' : value >= 65 ? 'FAIR' : 'CHECK';
  const label = level === 'GOOD' ? 'Dữ liệu tốt' : level === 'FAIR' ? 'Dữ liệu khá' : 'Cần xác minh';
  return { value, level, label };
}

/** Các tín hiệu rủi ro có thể kiểm chứng; tuyệt đối không suy diễn vi phạm. */
export function riskSignals(t) {
  const out = [];
  const st = statusOf(t);
  const d = daysToClose(t && t.closeDate);
  if (st === 'OPEN' && d !== null && d <= 1) {
    out.push({ code: 'DEADLINE', level: 'HIGH', label: d < 0 ? 'Đã quá hạn' : 'Hạn nộp rất sát' });
  } else if (st === 'OPEN' && d !== null && d <= 3) {
    out.push({ code: 'DEADLINE', level: 'MEDIUM', label: 'Còn không quá 3 ngày' });
  }
  const miss = missingFields(t);
  if (miss.length) out.push({ code: 'MISSING', level: miss.length >= 3 ? 'HIGH' : 'MEDIUM', label: miss.join(' · ') });
  if ((t && t.negHits || []).length) {
    out.push({ code: 'EXCLUDED', level: 'MEDIUM', label: `Có từ khóa hạn chế: ${t.negHits.slice(0, 2).join(', ')}` });
  }
  if (!t || !/^https:\/\/muasamcong\.mpi\.gov\.vn\//i.test(String(t.detailUrl || ''))) {
    out.push({ code: 'SOURCE', level: 'HIGH', label: 'Thiếu liên kết nguồn e-GP hợp lệ' });
  }
  return out;
}

/** Tóm tắt thay đổi gần nhất của một gói. */
export function changeSignal(t) {
  const rows = Array.isArray(t && t.changeLog) ? t.changeLog : [];
  const last = rows.length ? rows[rows.length - 1] : null;
  return { count: rows.length, last, changed: rows.length > 0 };
}
