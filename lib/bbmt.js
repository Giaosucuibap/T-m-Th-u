 ...
 */
function dayToMs(value, endOfDay = false) {
...
const startOfDayMs = (value) => dayToMs(value, false);
const endOfDayMs = (value) => dayToMs(value, true);
...
export function bbmtDateRange(scope = {}) {
  const from = startOfDayMs(scope.fromDate);
  const to = endOfDayMs(scope.toDate);
...
function bbmtStampMs(pkg) {
  const raw = pkg && (pkg.publicDateKqmt || pkg.bidRealityOpenDate || pkg.bidOpenDate);
  if (!raw) return null;
