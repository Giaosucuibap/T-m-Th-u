...
export function contractorsOfInvestor(packages) {
  ...
  /* Chỉ lập MST -> tên từ gói trúng độc lập có đúng một MST. Với liên danh,
     `winningCode` và `winningContractorName` không cùng thứ tự nên không được
     ghép hai mảng theo chỉ số. */
  const verifiedNames = new Map();
  for (const p of list) {
    const codes = [...new Set((p.winningTaxCodes || []).filter(Boolean))];
    ...
    const old = verifiedNames.get(codes[0]) || '';
    if (name.length > old.length) verifiedNames.set(codes[0], name);
  }
  ...
  for (const p of list) {
    ...
    for (const code of codes) {
      ...
      const nm = verifiedNames.get(code) || '';
      if (nm) row.names.add(nm);
      row.packages.push(p);
      if (isVenture) row.ventureCount += 1; else row.soloCount += 1;
    }
  }
  ...
  return [...rows.values()]
    .map((r) => {
      ...
      for (const p of r.packages) {
        ...
        const codes = [...new Set((p.winningTaxCodes || []).filter(Boolean))];
        const memberNames = Array.isArray(p.memberNames) ? p.memberNames : [];
        if (p.isVenture || codes.length > 1 || memberNames.length > 1) ventureValue += v;
        else soloValue += v;
      }
      return {
        taxCode: r.taxCode,
...
export function summarizeInvestor(packages, meta = {}) {
  ...
  return {
    ...
    discount: describe(list.map((p) => p.discountRate)),
    // Không biết tỷ lệ góp vốn nên HHI chỉ dùng giá trị trúng độc lập. Cộng toàn
    // bộ giá gói liên danh vào mỗi thành viên sẽ nhân giá trị và làm sai HHI.
    concentration: hhi(contractors.map((c) => c.soloValue)),
    concentrationScope: 'solo-only',
    ventureValueExcludedFromHhi: ventureValue,
    topContractor: top,
