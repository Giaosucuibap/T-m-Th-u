 ...
 *
 * Trả `{ xml, links }` — `links` là các ô cần gắn siêu liên kết, để buildXlsx
 * sinh thêm tệp quan hệ cho trang tính đó.
 */
function buildSheetXml({ columns = [], rows = [] }) {
  ...
  /* Ô kiểu `url` phải được ghi nhận để gắn siêu liên kết THẬT.
     Chỉ tô xanh gạch chân thì ô vẫn là chữ chết, bấm không đi đâu — dù phần
     chú thích đầu tệp này đã hứa "liên kết bấm được" như một ưu điểm so với
     CSV. Excel đòi ba thứ: phần tử <hyperlinks> trong trang tính, một quan hệ
     r:id, và tệp _rels riêng của trang tính. */
  const links = [];
  ...
  const sheet =
    ...
    // Thứ tự phần tử trong <worksheet> là BẮT BUỘC theo lược đồ OOXML:
    // <hyperlinks> phải nằm SAU <autoFilter>. Sai chỗ là Excel báo tệp hỏng.
    (links.length
      ? '<hyperlinks>' +
...
export function buildXlsx(spec = {}) {
  ...
  const files = [
    ...
    /* Trang tính có siêu liên kết cần một tệp quan hệ riêng, trỏ ra ngoài
