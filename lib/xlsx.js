 ...
 */
/** Giá trị này có phải một địa chỉ web mở được không? */
function isHttpUrl(value) {
  return /^https?:\/\/[^\s]+$/i.test(String(value === null || value === undefined ? '' : value).trim());
}

/**
 * Dựng XML một trang tính.
 *
 * Trả về `{ xml, links }` — `links` là danh sách ô cần gắn siêu liên kết, để
 * `buildXlsx` sinh thêm tệp quan hệ cho trang tính đó.
 */
function buildSheetXml({ columns = [], rows = [] }) {
  ...
  /* Ô kiểu `url` cần được ghi nhận để gắn siêu liên kết THẬT.
     Chỉ tô màu xanh gạch chân thôi thì ô vẫn là chữ chết, bấm không đi đâu —
     đúng lỗi của bản trước, dù phần chú thích đầu tệp đã hứa "liên kết bấm
     được". Excel yêu cầu ba thứ: phần tử <hyperlinks> trong trang tính, một
     quan hệ r:id, và tệp _rels riêng của trang tính đó. */
  const links = [];
  ...
  const bodyRows = data.map((r, ri) => {
    ...
    const cells = cols.map((c, i) => {
      ...
      if (type === 'url' && isHttpUrl(r[c.key])) links.push({ ref, target: String(r[c.key]).trim() });
  ...
  const sheet =
    ...
    // Thứ tự các phần tử trong <worksheet> là BẮT BUỘC theo lược đồ OOXML:
    // <hyperlinks> phải nằm SAU <autoFilter>. Đặt sai chỗ là Excel báo tệp hỏng.
    (links.length
      ? '<hyperlinks>' +
        links.map((l, i) => `<hyperlink ref="${l.ref}" r:id="rId${i + 1}"/>`).join('') +
        '</hyperlinks>'
      : '') +
    '</worksheet>';
...
export function buildXlsx(spec = {}) {
  ...
  const files = [
    ...
    /* Mỗi trang tính có siêu liên kết cần một tệp quan hệ riêng, trỏ ra ngoài
       (TargetMode="External"). Trang không có liên kết thì không sinh tệp. */
      ...
      ? [{
          ...
          data: utf8(
            ...
            sh.links.map((l, k) =>
              ...
              `Target="${xmlEscape(l.target)}" TargetMode="External"/>`).join('') +
            '</Relationships>')
        }]
      : []))
  ];
