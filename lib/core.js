 ...
 *
 *  Bản 4.0.0 thu hẹp `content_scripts.matches` từ toàn bộ e-GP xuống đúng các
 *  trang lựa chọn nhà thầu — đúng về quyền riêng tư. Nhưng phần nền vẫn dự
 *  phòng mở `/web/guest/home`, là trang KHÔNG còn content script. Hậu quả đo
 *  được trong Chromium: người dùng mới chưa lưu bộ lọc, bấm "Quét e-GP ngay"
 *  thì tab mở trang chủ e-GP, tầng nền nhắn cho một content script không tồn
 *  tại, và lượt quét chết với thông báo tiếng Anh của Chrome:
 *
 *      "Could not establish connection. Receiving end does not exist."
 *
 *  Hai hàm dưới đây là NGUỒN SỰ THẬT DUY NHẤT về phạm vi đó, để manifest và
 *  phần nền không thể lệch nhau lần nữa. Có kiểm thử đối chiếu thẳng với
 *  manifest.json trong tests/structure.test.js.
 * ------------------------------------------------------------------------- */
...
export function sanitizeRequestTemplate(request, sourcePageUrl='', candidateCount=0) {
  ...
  const scrub = (value, depth = 0) => {
    ...
    if (Array.isArray(value)) return value.slice(0,500).map((x) => scrub(x, depth + 1));
    if (value && typeof value === 'object') {
      const out = {};
      for (const [k,v] of Object.entries(value).slice(0,300)) {
        if (!secretKey.test(k)) out[k] = scrub(v, depth + 1);
      }
      return out;
    }
  ...
  let safeSource = 'https://muasamcong.mpi.gov.vn/vi/web/guest/contractor-selection?render=search';
  try {
    const src = new URL(sourcePageUrl);
    if (src.protocol === 'https:' && src.origin === 'https://muasamcong.mpi.gov.vn') safeSource = src.href;
  } catch {}
  return {
    url,
    method: 'POST',
