# Báo cáo đánh giá Giáo Sư Cùi Bắp 4.0.0
...
**Phiên bản nền:** 3.9.1  
**Phiên bản được đánh giá:** 4.0.0

## 1. Kết luận điều hành

Giáo Sư Cùi Bắp 3.9.1 có lợi thế thực dụng: bao phủ nhiều nghiệp vụ đấu thầu Việt Nam trong một extension, sử dụng nguồn e-GP chính thức và lưu dữ liệu cục bộ. Điểm yếu chính không nằm ở số lượng tính năng mà ở **chất lượng quyết định và độ an toàn vận hành**: dashboard chưa đóng vai trò buồng lái, chưa có pipeline Go/No-Go, thiếu lịch sử thay đổi, chuẩn hoá còn ca biên, tác vụ song song chưa cô lập và phân tích liên danh có thể gây sai tổng hợp.

Phiên bản 4.0.0 xử lý đúng các nút thắt đó bằng một lớp quyết định có cấu trúc, Radar thay đổi, chỉ số độ đầy đủ dữ liệu, URL/endpoint allowlist, request sanitization, cô lập tác vụ theo job/tab, nhập/sao lưu an toàn hơn và cách tính liên danh thận trọng. Bộ kiểm thử tự động đạt 44/44 tại thời điểm bàn giao.
...
**Kết luận:** 4.0.0 là một bản nâng cấp có cơ sở và đáng để chạy pilot/canary. Không có căn cứ để gọi bất kỳ extension phụ thuộc cổng e-GP nào là “100% hoàn hảo”; chất lượng phát hành cần được duy trì bằng kiểm thử live, giám sát thay đổi và quy trình rollback.

## 2. Phạm vi và phương pháp

Đánh giá dựa trên:

- so sánh mã nguồn/manifest giữa 3.9.1 và 4.0.0;
...
## 3. Đánh giá phiên bản 3.9.1

### 3.1. Ưu điểm

#### Bao phủ nghiệp vụ rộng

3.9.1 không chỉ là ô tìm kiếm. Phiên bản này đã có năm luồng lõi: tìm TBMT, tra kết quả theo mã số thuế, phân tích biên bản mở thầu/giảm giá, tìm KHLCNT và quét thị trường theo địa bàn. Các màn hình bổ sung về nhà thầu 360, chủ đầu tư, phân tích, bộ lọc lưu, lịch quét, thông báo và xuất Excel tạo thành một công cụ làm việc tương đối đầy đủ cho doanh nghiệp xây dựng.
...
| Vấn đề 3.9.1 | Tác động nghiệp vụ/kỹ thuật | Mức ưu tiên |
|---|---|---:|
...
## 4. 4.0.0 đã khắc phục như thế nào

| Hạn chế cũ | Thay đổi trong 4.0.0 | Giá trị mang lại | Rủi ro còn lại |
|---|---|---|---|
...
| Backup mang bí mật | Safe backup, cảnh báo full backup, import giới hạn/khử bí mật/tắt automation | An toàn hơn khi chia sẻ/chuyển máy | Full backup vẫn nhạy cảm; local storage chưa mã hoá |
...
| Full backup chứa bí mật | Có thể rò rỉ qua email/chat/cloud drive | Dán nhãn rõ, khuyến nghị safe backup, thêm mã hoá tuỳ chọn |
...
Kết quả tại thời điểm đánh giá: **44/44 kiểm thử đạt**.
...
- [ ] Xuất safe backup; giữ thư mục/ZIP 3.9.1 và hướng dẫn rollback.
...
- [ ] Mở tệp Excel, kiểm tra tên cột, Unicode, số tiền, ngày và các trường decision/owner/note/change.
- [ ] Mở safe backup dưới dạng văn bản; tìm `token`, `chatId`, `authorization` và xác nhận không có giá trị bí mật.
- [ ] Kiểm tra cảnh báo trước full backup.
...
Trước khi gọi 4.0.0 là bản ổn định:
...
- [ ] Đã kiểm tra import từ bản sao lưu 3.9.1 và rollback.
- [ ] Đã rà soát quyền, nội dung privacy, cảnh báo Telegram/full backup.
- [ ] Đã đo hiệu năng với tập dữ liệu đại diện.
- [ ] Có người chịu trách nhiệm theo dõi thay đổi e-GP và quy trình phát hành hotfix.

## 13. Kết luận cuối

4.0.0 cải thiện đúng bản chất của một phần mềm đấu thầu chuyên nghiệp: không chỉ “tìm nhiều hơn”, mà giúp người dùng biết **dữ liệu nào đáng tin đến đâu, việc gì cần làm tiếp, ai chịu trách nhiệm và nguồn nào phải kiểm tra**. Kiến trúc local-first và giới hạn nguồn chính thức là lợi thế đáng giữ.
